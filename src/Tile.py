...
class Tile:
    ...
    def render(self, surface: pygame.Surface, offset_x: int, offset_y: int) -> None:
        ...
        if self.powerup == "line":
            start_pos = (self.x + offset_x + 4, self.y + offset_y + 16)
            end_pos = (self.x + offset_x + settings.TILE_SIZE - 4, self.y + offset_y + 16)
            
            pygame.draw.line(surface, settings.COLOR_LINE, start_pos, end_pos, 4)
        elif self.powerup == "bomb":
            pygame.draw.circle(surface, settings.COLOR_BOMB, (self.x + offset_x + 16, self.y + offset_y + 16), 10)
